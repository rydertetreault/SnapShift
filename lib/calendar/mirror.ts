...
export async function mirrorSnapShiftEvents(): Promise<void> {
  ...
  for (let i = 0; i < updated.length; i++) {
    ...
    // Unmirrorable event (invalid dates even after repair): skip rather than
    // crashing the whole mirror pass. EventKit throws
    // "The start date must be before the end date." on inverted ranges.
    if (!range) continue;
